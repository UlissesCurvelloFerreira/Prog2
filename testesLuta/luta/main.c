...
// So uso aqui.
void show_fading_image(ALLEGRO_BITMAP *image, ALLEGRO_EVENT_QUEUE *event_queue, int src_x, int src_y, int src_w, int src_h, int x, int y, int ip)
{
    float alpha = 0.0f;
    int delayCount = 0;
    ...
    for (bool increasing = true; increasing || alpha > 0.0f;)
    {
        ...
        if (ev.type == ALLEGRO_EVENT_TIMER)
        {
            ...
            if (ip){
                al_draw_text(font1, al_map_rgb(255, 119, 0), SCREEN_WIDTH / 2, 50, ALLEGRO_ALIGN_CENTER, "GANHOU A LUTA");
            }
...
int main()
{
    ...
    int menuSelection = 0;
    //show_fading_image(background, queue, 900, 0, 900, 600, 0, 0, 0);
    while (1)
    {
