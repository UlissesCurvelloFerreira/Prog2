...
void escolher_mapas(Game *game, ALLEGRO_EVENT_QUEUE *queue, ALLEGRO_TIMER *timer)
{
    ...
    while (1)
    {
        ...
        //al_draw_text(font, al_map_rgb(255, 119, 0), SCREEN_WIDTH / 2, 50, ALLEGRO_ALIGN_CENTER, "SELECIONE O MAPA");
        
        //al_draw_bitmap_region(bolinha, 0 , 0 + (600 * game->mapa_selecionado), 900, 600, 0, 0, 0); /// mudar par aexibir os mapas dos raunds
        ...
        {
            ALLEGRO_COLOR color = al_map_rgb(255, 255, 255); // Cor padrão (branca)
            if (i == game->mapa_selecionado)
            {
                color = al_map_rgb(255, 0, 0); // Cor de seleção (vermelha)
            }

            int x = start_x + i * (square_size + spacing);
            al_draw_filled_rectangle(x, start_y, x + square_size, start_y + square_size, color);
        }
        if(game->mapa_selecionado == 3)
            al_draw_bitmap_region(bolinha, 1044, 7, 45, 37, 170, 480, 1);
        if(game->mapa_selecionado == 0)
            al_draw_bitmap_region(bolinha, 1044, 7, 45, 37, 690, 480, 0);
        ...
        if (event.type == ALLEGRO_EVENT_KEY_DOWN)
        {
            if (!player1_selected)
            {
                ...
                {
                    if (game->mapa_selecionado < 3)game->mapa_selecionado += 1;
                }
...
void soco(Player *p)
{
    ...
    {
        p->ataque = 1;
        if (p->control->down) p->animacao_atual = 6; // Soco baixo.
        else if (p->no_ar) p->animacao_atual = 14; // soco cima.
        else p->animacao_atual = 4; // Soco.
...
void chute(Player *p)
{
    ...
    {
        p->ataque = 1;
        if (p->control->down) p->animacao_atual = 7; // Chute baixo
        else if (p->no_ar) p->animacao_atual = 15; // chute cima
        else p->animacao_atual = 5; // Chute
        p->animations[p->animacao_atual].current_frame = 0;
    }
}
...
void leva_dano(Player *p)
{
    ...
    {
        p->ataque = 1;
        if (p->control->down)p->animacao_atual = 11; // animação de dano abaixado.
        else p->animacao_atual = 10; // animação de dano.
        p->animations[p->animacao_atual].current_frame = 0;
    }
}
...
void movimento_defesa(Player *p)
{
    ...
    if (p->control->defesa)
    {
        if (p->control->down) p->animacao_atual = 12; // defesa baixo.
        else if (p->no_ar) p->animacao_atual = 13; // defesa cima.
        else p->animacao_atual = 8; // defesa cima.
    }
}
    ...
    */
    ...
    while (1)
    {
        ...
        if (event.type == ALLEGRO_EVENT_TIMER && event.timer.source == local_timer)
        {
            ...
            // fatality
            if (game->player1->control->fire  && !game->player1->no_ar && game->player2->vida < 20){game->player1->animacao_atual = 9;}
            if (game->player2->control->fire  && !game->player2->no_ar && game->player1->vida < 20){game->player2->animacao_atual = 9;}
            ...
            //fatality
            if (game->player2->vida <= 20){al_draw_bitmap_region(bolinha, 33, 1203, 26, 24, 1, 13, 0);}
            if (game->player1->vida <= 20){al_draw_bitmap_region(bolinha, 33, 1203, 26, 24, 900 - 26, 13, 0);}
